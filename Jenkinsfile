		...
		stages {                 
			...
			stage('Deploy') {                         
				steps {                                 
					...
					sh 'if [ "$(docker service ps -q movieUI)" == "no such service: moviesUI" ]; then docker service create --replicas 1 -p 3030:3030 --env BACKEND_URL=172.23.9.232:3000 --name movieUI cristiancristancho/rampup_front:latest; else docker service update --replicas 1 --env-add BACKEND_URL=172.23.9.232:3000 cristiancristancho/rampup_front:latest; fi'
				}                 
			}         
		} 
} 
